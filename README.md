# Prompt-Stack

> **The AI-native full-stack template that gets you from idea to production in 4 minutes**

🚀 **NEW: CLI Tool Available!** Skip setup entirely with our command-line interface.

```bash
# Install CLI
npm install -g prompt-stack-cli

# Create new project
prompt-stack create my-saas-app
cd my-saas-app

# Start development
prompt-stack dev
# ✅ Running at http://localhost:3000 (14 seconds)
```

## 🎯 What You Get

**Instant AI-powered SaaS starter** with:
- ✅ **Authentication** - Supabase with role-based access
- ✅ **AI Integration** - OpenAI, Anthropic, DeepSeek, Gemini
- ✅ **Payments** - Stripe & Lemon Squeezy ready
- ✅ **Database** - PostgreSQL with vector search
- ✅ **Frontend** - Next.js 15 with TypeScript
- ✅ **Backend** - FastAPI with async/await
- ✅ **Docker** - One-command development environment

## 🚀 Quick Start Options

### Option 1: CLI Tool (Recommended)
```bash
npm install -g prompt-stack-cli
prompt-stack create my-app
cd my-app && prompt-stack dev
```

### Option 2: Manual Setup
```bash
git clone https://github.com/your-org/prompt-stack.git
cd prompt-stack/template
./setup.sh && make dev
```

## 📊 Performance

- **Demo Mode**: 14 seconds to running app
- **Production Setup**: 4-5 minutes total
- **CLI vs Manual**: 30 seconds vs 3-5 minutes

## 🛠️ Tech Stack

**Frontend:**
- Next.js 15 (App Router)
- TypeScript
- Tailwind CSS
- Supabase Auth

**Backend:**
- FastAPI (Python)
- Supabase (PostgreSQL + Auth)
- Vector Search (pgvector)
- Multiple AI Providers

**Infrastructure:**
- Docker & Docker Compose
- Ready for Railway, Render, Vercel
- GitHub Actions CI/CD

## 📚 Documentation

- [Getting Started](./template/docs/GETTING_STARTED.md)
- [Production Setup Guide](./template/PRODUCTION_SETUP_GUIDE.md)
- [Architecture Overview](./template/docs/ARCHITECTURE_OVERVIEW.md)
- [Development Patterns](./template/docs/DEVELOPMENT_PATTERNS.md)

## 🤝 Contributing

See [CONTRIBUTING.md](./template/CONTRIBUTING.md) for development setup and contribution guidelines.

## 📄 License

MIT License - see [LICENSE](./template/LICENSE) for details.

---

**Built for developers who want to ship fast** 🚀